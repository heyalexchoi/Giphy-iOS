...
describe(@"AXCGiphy", ^{
    ...
    it(@"should get translation result", ^AsyncBlock {
        [AXCGiphy giphyTranslationForTerm:@"cow" completion:^(AXCGiphy *result, NSError *error) {
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    it(@"translation result should produce a gif with all these properties", ^AsyncBlock{
         [AXCGiphy giphyTranslationForTerm:@"cow" completion:^(AXCGiphy *result, NSError *error) {
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                    AXCGiphy * gif = result;
                    expect(gif.gifID.length).to.beGreaterThan(0);
    ...
    it(@"should get a gif for id", ^AsyncBlock {
        [AXCGiphy gifForID:@"slNwi1TTwR40U" completion:^(AXCGiphy *result, NSError *error) {
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                expect(result).toNot.beNil();
                done();
            }];
    ...
    it(@"gif for id result should produce a gif with all these properties", ^AsyncBlock{
        [AXCGiphy gifForID:@"slNwi1TTwR40U" completion:^(AXCGiphy *result, NSError *error) {
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                AXCGiphy * gif = result;
                expect(gif.gifID.length).to.beGreaterThan(0);
    ...
    it(@"should get gifs for IDs ", ^AsyncBlock {
        [AXCGiphy gifsForIDs:@[@"OzmIjVRDwM6as",@"feqkVgjJpYtjy"] completion:^(NSArray *results, NSError *error) {
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
    ...
    it(@"gifs for IDs results should produce gifs with all these properties", ^AsyncBlock{
         [AXCGiphy gifsForIDs:@[@"OzmIjVRDwM6as",@"feqkVgjJpYtjy"] completion:^(NSArray *results, NSError *error) {
            [[NSOperationQueue mainQueue] addOperationWithBlock:^{
                [results enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
                    AXCGiphy * gif = obj;
